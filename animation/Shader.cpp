...
void CBoundingBoxShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE d3dPrimitiveTopologyType)
{
	...
	CShader::CreateShader(pd3dDevice, pd3dGraphicsRootSignature, 1, &rtvFormat, DXGI_FORMAT_D32_FLOAT, d3dPrimitiveTopologyType);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CObjectsShader::CObjectsShader(int nObjects, CGameObject** ppObjects, CGameObject* pPlayer)
{
	this->m_nObjects = nObjects;
	this->m_ppObjects = ppObjects;
	this->m_pPlayer = pPlayer;

	this->m_ppObjects[this->m_nObjects - 1] = this->m_pPlayer;
}
...
// ** 힙에 상수 버퍼 리소스 생성 + CPU에서 접근 가능하도록 매핑
void CDepthRenderShader::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	// ** D3D12_HEAP_TYPE_UPLOAD <- 이 힙에 버퍼 리소스를 만들기 위한 메모리 공간의 크기 정해주는 부분
	// ** GPU가 메모리를 256바이트 단위로 정렬하기 때문에 256의 배수 크기로 맞춰주는 것
	ncbDepthElementBytes = ((sizeof(TOLIGHTSPACES) + 255) & ~255); //256의 배수

	// ** 상수 버퍼 리소스(m_pd3dcbToLightSpaces) 생성
	// ** 이 리소스는 D3D12_HEAP_TYPE_UPLOAD에 위치, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER 타입으로 생성됨
	...
	// ** 생성한 상수 버퍼에 CPU가 접근할 수 있도록 매핑해줌
	// ** m_pcbMappedToLightSpaces가 매핑된 메모리의 포인터 -> 얘로 CPU에서 m_pd3dcbToLightSpaces에 접근 가능
...
{
	// ** m_pToLightSpaces(구조체)의 데이터를 m_pcbMappedToLightSpaces(상수버퍼)에 복사해줌
...
void CDepthRenderShader::PrepareShadowMap(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera/*, BoundingBox* pxmSceneBoundingBox*/)
{
	for (int j = 0; j < MAX_LIGHTS; j++)
	{
		if (m_pLights[j].m_bEnable)
		{
			...
			// ** m_pToLightSpaces는 그림자 매핑을 위한 조명의 변환 행렬과 위치를 저장하는 역할 -> 겁나 중요함
			m_pToLightSpaces->m_pToLightSpaces[j].m_xmf4Position = XMFLOAT4(xmf3Position.x, xmf3Position.y, xmf3Position.z, 1.0f);

			// ** m_pDepthFromLightTexture는 그림자 매핑을 구현하기 위한 중요한 텍스처
			// ** 조명의 시점에서 본 씬의 깊이 정보를 저장하고, 이를 이용하여 각 픽셀이 조명에 의해 가려지는지 여부를 결정하여 그림자를 정확하게 그릴 수 있도록 해줌

			// ** m_pDepthFromLightTexture 리소스의 상태를 D3D12_RESOURCE_STATE_COMMON에서 D3D12_RESOURCE_STATE_RENDER_TARGET로 변경
			// ** 렌더타겟으로 변경해서 해당 광원의 시점에서 씬을 렌더링할 수 있도록 하기 위함
			...
			// ** 리소스의 상태를 다시 D3D12_RESOURCE_STATE_RENDER_TARGET에서 D3D12_RESOURCE_STATE_COMMON로 바꿔줌
