...
void CShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera , int m_nPipelineStates)
{
	...
	if (m_nPipelineStates != -1) OnPrepareRender(pd3dCommandList);
}
...
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "VSSkyBox", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CSkyBoxShader::CreatePixelShader()
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "PSSkyBox", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "PSTexturedStandardMultipleRTs", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "VSSkinnedAnimationStandard", "vs_5_1", &m_pd3dVertexShaderBlob));
}
...
{
	CStandardShader::Render(pd3dCommandList, pCamera, 0);
...
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "VSScreenRectSamplingTextured", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CPostProcessingShader::CreatePixelShader()
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "PSScreenRectSamplingTextured", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void CPostProcessingShader::OnPrepareRenderTarget(ID3D12GraphicsCommandList* pd3dCommandList, int nRenderTargets, D3D12_CPU_DESCRIPTOR_HANDLE* pd3dRtvCPUHandles, D3D12_CPU_DESCRIPTOR_HANDLE pd3dDsvCPUHandle)
{
	...
	for (int i = 0; i < nRenderTargets; i++)
	{
		...
		pd3dCommandList->ClearRenderTargetView(pd3dRtvCPUHandles[i], Colors::Red, 0, NULL);
	}
...
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "VSScreenRectSamplingTextured", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CTextureToFullScreenShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "PSScreenRectSamplingTextured", "ps_5_1", ppd3dShaderBlob));
}
...
void CBoundingBoxShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE d3dPrimitiveTopologyType)
{
	...
	//Shader::CreateShader(pd3dDevice, pd3dGraphicsRootSignature, 1, &rtvFormat, DXGI_FORMAT_D32_FLOAT, d3dPrimitiveTopologyType);
...
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "VSBoundingBox", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CBoundingBoxShader::CreatePixelShader()
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "PSBoundingBox", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void CBlurShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, int nPipelineState)
{
	...
	m_d3dPipelineStateDesc.SampleMask = UINT_MAX;
	m_d3dPipelineStateDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
	m_d3dPipelineStateDesc.NumRenderTargets = DEFERREDNUM;
	for (UINT i = 0; i < DEFERREDNUM; i++)
		m_d3dPipelineStateDesc.RTVFormats[i] = pdxgiResourceFormats[i];
	m_d3dPipelineStateDesc.DSVFormat = DXGI_FORMAT_D32_FLOAT;
	m_d3dPipelineStateDesc.SampleDesc.Count = 1;
	m_d3dPipelineStateDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
...
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "PSBlur", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void ShadowMap::BuildDescriptors(D3D12_CPU_DESCRIPTOR_HANDLE hCpuSrv,
...
void ShadowMap::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	::ZeroMemory(&m_d3dPipelineStateDesc, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));
	m_d3dPipelineStateDesc.pRootSignature = pd3dGraphicsRootSignature;
	m_d3dPipelineStateDesc.VS = CreateVertexShader();
...
D3D12_INPUT_LAYOUT_DESC ShadowMap::CreateInputLayout()
{
	UINT nInputElementDescs = 1;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_RASTERIZER_DESC ShadowMap::CreateRasterizerState()
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
	//	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_WIREFRAME;
...
D3D12_BLEND_DESC ShadowMap::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
D3D12_DEPTH_STENCIL_DESC ShadowMap::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = TRUE;
...
D3D12_SHADER_BYTECODE ShadowMap::CreateVertexShader()
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "VSShadowMap", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE ShadowMap::CreatePixelShader()
{
	return(CompileShaderFromFile(L"Shaders.hlsl", "PSShadowMap", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void ShadowMap::BuildDescriptors()
{
	...
	srvDesc.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS; // SRV 포맷
	...
	mShadowMap->SetName(L"ShadowMapResource");
